...
static void rc_data_initialize(rc_t *rc)
{
    ...
    case RC_MODE_TX:
        ...
        if (air_io_get_bound_addr(&rc->outputs.air.air, &addr))
        {
            ...
            if (config_get_air_info(&air_info, &addr))
            {
...
void rc_set_peer_air_lora_config(rc_t *rc, air_addr_t *addr, air_lora_band_e band, air_lora_supported_modes_e modes)
{
    ...
    air_info_t info;
    if (!config_get_air_info(&info, addr))
    {
...
static void rc_reconfigure_input(rc_t *rc)
{
    ...
    case RC_MODE_TX:
        ...
        case TX_INPUT_CRSF:
            ...
            input_config.crsf.pin_num = settings_get_key_pin_num(SETTING_KEY_TX_CRSF_PIN);
    ...
    case RC_MODE_RX:
    {
        ...
        rc_get_air_lora_config(rc, &lora);
        if (rc->state.bind_active)
        {
...
static void rc_reconfigure_output(rc_t *rc)
{
    ...
    case RC_MODE_RX:
        ...
        case RX_OUTPUT_SBUS_SPORT:
            ...
            output_config.sbus.sbus_pin_num = settings_get_key_pin_num(SETTING_KEY_RX_SBUS_PIN);
            output_config.sbus.sbus_inverted = settings_get_key_bool(SETTING_KEY_RX_SBUS_INVERTED);
            output_config.sbus.sport_pin_num = settings_get_key_pin_num(SETTING_KEY_RX_SPORT_PIN);
        ...
        case RX_OUTPUT_MSP:
            ...
            output_config.msp.tx_pin_num = settings_get_key_pin_num(SETTING_KEY_RX_MSP_TX_PIN);
            output_config.msp.rx_pin_num = settings_get_key_pin_num(SETTING_KEY_RX_MSP_RX_PIN);
        ...
        case RX_OUTPUT_CRSF:
            ...
            output_config.crsf.tx_pin_num = settings_get_key_pin_num(SETTING_KEY_RX_CRSF_TX_PIN);
            output_config.crsf.rx_pin_num = settings_get_key_pin_num(SETTING_KEY_RX_CRSF_RX_PIN);
        ...
        case RX_OUTPUT_FPORT:
            ...
            output_config.fport.tx_pin_num = settings_get_key_pin_num(SETTING_KEY_RX_FPORT_TX_PIN);
            output_config.fport.rx_pin_num = settings_get_key_pin_num(SETTING_KEY_RX_FPORT_RX_PIN);
...
static void rc_update_binding(rc_t *rc)
{
    ...
    if (air_io_has_bind_request(air_io, &packet, &needs_confirmation) &&
        (rc->state.accept_bind || !needs_confirmation))
    {
        ...
        config_set_air_info(&pairing.addr, &packet.info);
...
static void rc_setting_changed(const setting_t *setting, void *user_data)
{
    ...
    else
    {
        ...
        case RC_MODE_RX:
            ...
            {
                // XXX: Config was changed, but we need to inform the other
                // end. For now, assume they're available via P2P.
                air_addr_t pair_addr;
                if (rc_get_pair_addr(rc, &pair_addr))
                {
                    rc_rmp_send_air_lora_config(&rc->state.rc_rmp, &pair_addr, true);
                }
                rc_invalidate_input(rc);
            }
            break;
...
bool rc_has_pending_bind_request(rc_t *rc, air_bind_packet_t *packet)
{
    if (!rc->state.invalidate_input && !rc->state.invalidate_output && rc->state.bind_active)
    {
        ...
        if (air_io)
        {
            ...
            return air_io_has_bind_request(air_io, packet, &needs_confirmation) && needs_confirmation;
        }
    }
    return false;
}
...
void rc_update(rc_t *rc)
{
    ...
    time_micros_t now = time_micros_now();
    rc->state.dirty |= input_update(rc->input, now);
